function OptRes = func_dc_energyflow_fpp(P_diff_rating_mat_in, Bat_info, CL, ENV)
...
P_direct_mat = zeros(size(CL.Conn.direct));           % no direct energy dilivery   
...
P_fpe_lim = ENV.Fpp_Conv.e_lim_singlevar;  % energy limits of var converters
P_var_lim = ENV.Var_Conv.e_lim_singlevar;  % 0, energy limits of direc-energy-processing converters
...
P_var = optimvar('P_var',Node_num, Node_num);    % 3*3 dimensional energy transfer variables, secondary layer
...
% Construct the full-energy-processing converter energy limits
fpp_conv_con = optimconstr(0);
for i = 1:Node_num
    for j = i:Node_num
        if (P_fpp_mat(i,j) == 1)
            fpp_conv_con(end+1) = P(i,j) <= P_fpe_lim;
            fpp_conv_con(end+1) = -P_fpe_lim <= P(i,j);
%            Conv_energy_rating_partition_mat(i,j) = ENV.Avg_Conv.e_lim(temp_ct);
        end
    end
end
...
if(exitflag == 1)
    ...
    OptRes.P_full_energy_processing = P_fpp_mat;
    ...
    [OptRes.Avg_energy_flow, OptRes.Fpp_energy_flow] = func_energy_flow_decomp(Total_energy_flow, CL);
    ...
    OptRes.Fpp_energy_process = sum(sum(abs(OptRes.Fpp_energy_flow)))/2;
    OptRes.Total_energy_process = OptRes.Fpp_energy_process + OptRes.Var_energy_process + OptRes.Avg_energy_process;
    ...
    for i = 1: CL.Stat.Bat_num
        OptRes.BatOutputenergy(i) = sum((OptRes.Var_energy_flow(CL.Bat{i}.ind,:))) + ...
         sum((OptRes.Avg_energy_flow(CL.Bat{i}.ind,:))) + sum((OptRes.Fpp_energy_flow(CL.Bat{i}.ind,:)));
    end
    ...
    error('Error in dc energy flow optimization on two-layer converters\n');
end
