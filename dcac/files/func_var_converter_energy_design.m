function ExpRes = func_var_converter_energy_design(CL,ENV,bool_sort)
...
for k0 = 1:ENV.Sweep.Stat.Bat                  % k_0 loop for 8 battery variance, n of bat. types
    [CL, Bat_Info_MC] = Update_k0_loop(k0,CL,ENV,bool_sort);    % 9*100 for each bat. type, qlim var  
...
function ENV = Update_k1_loop(k1, ENV)
    ENV.Var_Conv.e_lim_singlevar = ENV.Sweep.Conv.e_lim(k1);
end
