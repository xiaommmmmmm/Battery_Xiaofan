function ExpRes = func_opt_gap(CL,ENV)
...
for k0 = 1:ENV.Sweep.Stat.Bat-2                 % k_0 loop for 1:6 battery variance
    ...
    for k1 = 1:ENV.Sweep.Stat.Conv             % k_1 loop for 20 power converter rating
        ...
        parfor k2 = 1:ENV.Var_Conv.MC_trial       % k_2 loop is for MC simu, please
            ...
            OutVar_sub = func_dc_powerflow_opt_gap(OutVar.Avg_power_flow, OutVar.Var_power_flow, Bat_Info_MC(:,k2), CL, ENV);
...
%             bat_power_eff_error(k0,k1,k2) = abs(OutVar.powerEff-OutVar.powerEffProxy);
            ...
            max_output_power_sub(k0,k1,k2) = abs(OutVar_sub.Maximum_output_power);
            bat_uratio_power_sub(k0,k1,k2) = abs(OutVar_sub.Maximum_output_power)/sum(OutVar_sub.Bat_power_rating);
...
ExpRes.bat_uratio_power_sub = bat_uratio_power_sub;
ExpRes.max_output_power_sub = max_output_power_sub;

ExpRes.bat_uratio_power_gap = bat_uratio_power - bat_uratio_power_sub;
ExpRes.max_output_power_gap = max_output_power - max_output_power_sub;
...
function ENV = Update_k1_loop(k1, ENV)
    ENV.Var_Conv.p_lim_singlevar = ENV.Sweep.Conv.p_lim(k1);
end
