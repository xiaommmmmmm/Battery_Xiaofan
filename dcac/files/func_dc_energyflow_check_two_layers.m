function OptRes = func_dc_energyflow_check_two_layers(P_diff_rating_mat_in, Bat_info, IniDes, CL, ENV)

% ENV.Avg_Conv.e_lim_mat = P_diff_mat_in;
Conv_energy_rating_partition_mat = zeros(CL.Stat.Node_num,CL.Stat.Node_num); %11
...
E_var_lim = ENV.Var_Conv.e_lim;  % energy limits of var converters
...
lambda_shrink = optimvar('lambda_shrink',1,1);      % shrinking factors in time 
...
calculated_energy_flow_con = optimconstr(0);
for i = 1:Node_num
    for j = 1:Node_num
        if (P_diff_var_mat(i,j) == 1)
            calculated_energy_flow_con(end+1) = P_var(i,j) == IniDes.Var_energy_flow(i,j)*lambda_shrink;
        end
    end
end
...
for i = 1:Node_num
    for j = 1:Node_num
        if (P_diff_avg_mat(i,j) == 1)
            calculated_energy_flow_con(end+1) = P(i,j) == IniDes.Avg_energy_flow(i,j)*lambda_shrink;
        end
    end
end
prob.Constraints.calculated_energy_flow_con = calculated_energy_flow_con;
...
if(exitflag == 1)
    ...
    OptRes.Total_energy_flow = reshape(OptRes.sol(1:Node_num*Node_num),[Node_num,Node_num]);
    [OptRes.Avg_energy_flow, OptRes.Direct_energy_flow] = func_energy_flow_decomp(OptRes.Total_energy_flow, CL);
    ...
    OptRes.Var_conv_energy_rating = ones(1,CL.Stat.VarConn_num)*ENV.Var_Conv.e_lim;
    OptRes.Bat_energy_rating = Bat_energy_rating;
    OptRes.Output_q_max = sol((2*Node_num*Node_num + 1):(2*Node_num*Node_num + 1));
    if (sol(end) == sol(2*Node_num*Node_num + 2))
        OptRes.lambda_shrink = sol((2*Node_num*Node_num + 2):(2*Node_num*Node_num + 2));
    else
        error('Error in interpreting the results in energy flow optimization of two layers');
    end
else
    error('Error in designing energy flows of two layers');
end
