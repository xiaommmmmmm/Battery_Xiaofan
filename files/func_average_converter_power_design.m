...
for i = 1:min(trial_num,exhaust_num)
    ...
    OptRes(i) = func_dc_powerflow_avg_layer(P_diff_mat_in + P_diff_mat_in', CL, ENV);
end
%% Pick the Optimal Data
neg_max_p_out = [];
for i = 1:min(trial_num,exhaust_num)
    neg_max_p_out = [neg_max_p_out,OptRes(i).bus_plim];
end
OptAvg_power_collection = {};
for i = 1:min(trial_num,exhaust_num)
    if (OptRes(i).bus_plim == min(neg_max_p_out))
        
        OptAvg_power_collection{end+1}.output_power = abs(OptRes(i).bus_plim);
        OptAvg_power_collection{end}.Avg_power_flow = OptRes(i).Avg_power_flow;
        OptAvg_power_collection{end}.Direct_power_flow = OptRes(i).Direct_power_flow;
