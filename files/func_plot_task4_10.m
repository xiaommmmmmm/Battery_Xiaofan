function y = func_plot_task4_10(ENV,ExpTrajRes,ExpTrajRes_trad)
...
for kn1 = 1:ENV.Sweep.Stat.EV_Mu                       % k_n1 loop for ev demand mean      
    for k0 = 1:ENV.Sweep.Stat.EV_Var                   % k_0 loop for ev demand variance      
        for k1 = 1:ENV.Sweep.Stat.Td_Mu                % k_1 loop for Tdelay mu 
            charging_power_matrix(kn1,k0,k1) = ExpectedChargingPower(ExpTrajRes, ENV, kn1, k0, k1);
            charging_power_trad_matrix(kn1,k0,k1) = ExpectedChargingPower(ExpTrajRes_trad, ENV, kn1, k0, k1);
        end
    end
end
...
for k0 = 1:ENV.Sweep.Stat.EV_Var
    pic(k0) = plot(ENV.Sweep.Td_Mu.mu, reshape(charging_power_matrix(2,k0,:),1,ENV.Sweep.Stat.Td_Mu),'-s','linewidth',2);
    hold on;
end
for k0 = 1:ENV.Sweep.Stat.EV_Var
    pic2(k0) = plot(ENV.Sweep.Td_Mu.mu, reshape(charging_power_matrix(1,k0,:),1,ENV.Sweep.Stat.Td_Mu),'--s','linewidth',2);
    hold on;
end
xlabel('Td Mean (h)');
...
figure();
for k0 = 1:ENV.Sweep.Stat.EV_Var
    pic(k0) = plot(ENV.Sweep.Td_Mu.mu, reshape(charging_power_trad_matrix(2,k0,:),1,ENV.Sweep.Stat.Td_Mu),'-s','linewidth',2);
    hold on;
end
for k0 = 1:ENV.Sweep.Stat.EV_Var
    pic2(k0) = plot(ENV.Sweep.Td_Mu.mu, reshape(charging_power_trad_matrix(1,k0,:),1,ENV.Sweep.Stat.Td_Mu),'--s','linewidth',2);
    hold on;
end
xlabel('Td Mean (h)');
ylabel('Charging Time (kW)');
legend([pic,pic2],'StdEV = 10% AvgEV = 50 kWh','StdEV = 30% AvgEV = 50 kWh','StdEV = 50% AvgEV = 50 kWh',...
'StdEV = 10% AvgEV = 33 kWh','StdEV = 30% AvgEV = 33 kWh','StdEV = 50% AvgEV = 33 kWh'...
);
title('PPP');
...
function charging_power_mean = ExpectedChargingPower(ExpTrajRes, ENV, kn1, k0, k1)
    charging_power_vec = [];
    for k2 = 1:ENV.Var_Conv.MC_trial
        for k3 = 1:ENV.Sweep.Stat.EV_MC_trial
            for k4 = 1:ENV.Sweep.Stat.Td_MC_trial
                ...
                charging_power_vec = [charging_power_vec, charging_power];
            end
        end
    end
    charging_power_mean = mean(charging_power_vec);
end
